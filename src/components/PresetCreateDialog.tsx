      ...
      <div className="bg-[var(--moon-surface)] rounded-2xl border border-[var(--moon-border)] w-full max-w-2xl shadow-xl overflow-hidden">
        <div className="px-6 pt-5 pb-4 border-b border-[var(--moon-border)] flex items-start justify-between">
          <div>
            ...
            <p className="text-[var(--moon-muted)] text-sm mt-1">
              {creationMode === 'active' ? 'Save your current active mods as a preset.' : 'Pick specific mods to include in this preset.'}
            </p>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div>
            ...
            />
            <p className="text-xs text-[var(--moon-muted)] mt-1">Give your preset a short, memorable name.</p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 bg-[var(--moon-surface-elevated)] p-1 rounded-lg border border-[var(--moon-border)]">
            <button
              ...
              className={`px-3 py-2 rounded-md text-sm font-medium transition ${creationMode === 'active' ? 'bg-[var(--moon-accent)] text-white' : 'text-[var(--moon-text)] hover:bg-[var(--moon-surface)]'}`}
            >
            ...
            <button
              ...
              className={`px-3 py-2 rounded-md text-sm font-medium transition ${creationMode === 'select' ? 'bg-[var(--moon-accent)] text-white' : 'text-[var(--moon-text)] hover:bg-[var(--moon-surface)]'}`}
            >
          ...
          {creationMode === 'select' && (
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                ...
                />
                <span className="text-xs text-[var(--moon-muted)] whitespace-nowrap">{filteredMods.length} {filteredMods.length === 1 ? 'mod' : 'mods'}</span>
              </div>
              <div className="max-h-72 overflow-y-auto rounded-lg border border-[var(--moon-border)]">
                {filteredMods.length === 0 ? (
                  <div className="p-6 text-center text-[var(--moon-muted)] text-sm">No mods match your search.</div>
                ) : (
                  <ul className="divide-y divide-[var(--moon-border)]">
                    {filteredMods.map(mod => (
                      <li key={mod.id} className="flex items-center justify-between px-4 py-3 hover:bg-[var(--moon-surface)] transition">
                        <div className="min-w-0">
                          <div className="text-sm text-[var(--moon-text)] truncate" title={mod.title}>{mod.title}</div>
                          <div className="text-xs text-[var(--moon-muted)] truncate">
                            {mod.character ? `Character: ${mod.character}` : 'Other Mods'}
                          </div>
                        </div>
                        <input
                          type="checkbox"
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            >
              {loading ? (presetToEdit ? 'Saving...' : 'Creating...') : (presetToEdit ? 'Save Changes' : 'Create Preset')}
            </button>
          </div>
        </form>
      </div>
