...
const Sidebar: React.FC<SidebarProps> = ({ currentPage, onPageChange }) => {
  ...
  return (
    <div
      className={`${
        expanded ? "w-64" : "w-16"
      } h-screen bg-gray-900/50 backdrop-blur-sm border-r border-gray-800/50 flex flex-col transition-all duration-300`}
    >
      {/* Header */}
      <div
        className={`border-b border-gray-800/50 ${expanded ? "p-4" : "p-4"}`}
      >
        <div
      ...
      <nav className={`${expanded ? "p-4" : "p-2"} flex-1 space-y-1`}>
        {navigationItems.map((item) => (
          <button
            ...
            title={item.label}
          >
            <span className={`${expanded ? "text-lg mr-3" : "text-lg mb-1"}`}>
              {item.icon}
            </span>
            <span
              className={`${
                expanded ? "text-sm font-medium" : "text-xs font-medium"
              }`}
            >
              {item.label}
            </span>
      ...
      {/* Footer */}
      <div
        className={`${expanded ? "p-4" : "p-2"} border-t border-gray-800/50`}
      >
        <div
          className={`flex ${
            expanded
